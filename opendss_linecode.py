...
class TParser:
    def parse_as_matrix(self, expected_order, matrix_buffer):
        if self.FAutoIncrement:
            self.get_next_param()
        row_buf = None
        try:
            row_buf = np.zeros(expected_order)
            matrix_buffer.fill(0.0)
            for i in range(expected_order):
                elements_found = self.parse_as_vector(expected_order, row_buf)
                # Returns matrix in Column Order (Fortran order)
                k = i
                for j in range(elements_found):
                    matrix_buffer[k] = row_buf[j]
                    k += expected_order
        except Exception as e:
            self.dss_message_dlg(f'Matrix Buffer in ParseAsMatrix Probably Too Small: {str(e)}', True)
        finally:
            if row_buf is not None:
                del row_buf
        return expected_order

    def parse_as_sym_matrix(self, expected_order, matrix_buffer):
        def element_index(ii, jj):
            return (jj - 1) * expected_order + ii
        ...
        try:
            ...
            for i in range(expected_order):
                ...
                # Returns matrix in Column Order (Fortran order)
                for j in range(elements_found):
                    matrix_buffer[element_index(i, j)] = row_buf[j]
                    if i != j:
                        matrix_buffer[element_index(j, i)] = row_buf[j]
        except Exception as e:
            self.dss_message_dlg(f'Matrix Buffer in ParseAsSymMatrix Probably Too Small: {str(e)}', True)
        finally:
            if row_buf is not None:
                del row_buf
        return expected_order
